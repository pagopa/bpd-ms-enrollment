...
public interface CitizenRestClient {
    ...
    @GetMapping(value = "${rest-client.citizen.findById.url}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    CitizenResource findById(@PathVariable("id") String fiscalCode);

    @PutMapping(value = "${rest-client.citizen.update.url}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    CitizenResource update(@NotBlank @PathVariable("id") String fiscalCode, @RequestBody @Valid CitizenDto dto);

    @DeleteMapping(value = "${rest-client.citizen.delete.url}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
